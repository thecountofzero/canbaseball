steal('jquery',
	'remotesupport/lib/remotesupport')
	
.then(function($) {

	REMOTESUPPORT.namespace('REMOTESUPPORT.localization');
	
	REMOTESUPPORT.localization = (function() {
		
		var defaults = {
			fallbackLang: "en-US",
			localesPath: "/remotesupport/locales",
			keyseparator: ".",
			enable$t: true,
			dictionary: false,
			lang: false,
		...
		function _detectLanguage() {
			if(navigator) {
				return _normaliseLang(navigator.language || navigator.userLanguage);
			}
			else {
		...
		function translateModelStr(key) {
			var keys = key.split("|");
			return _translate(keys[0],(keys.length===2?keys[1]:""));
		}
		...
		function loadDictionary(dictionaryPath, callback) {
			...
			return (function() {
				...
				_loadDictionary = function(lang, doneCallback) {
					//steal.dev.log("Retries (" + dictionaryPath + ") : " + retries);
		...
		return {
			...
			translateModelStr: translateModelStr
		};
